...
上一次整活还是在上一个月，写了一个简单的module并且熟悉了module的[挂载](https://so.csdn.net/so/search?q=挂载&spm=1001.2101.3001.7020)查看和卸载。这一次我们自然玩一个大的，就是利用linux的debugfs API写一个调试文件系统。

事实上，底层的[API](https://so.csdn.net/so/search?q=API&spm=1001.2101.3001.7020)全写好了，我们就是简单的调调API就成的事情！
