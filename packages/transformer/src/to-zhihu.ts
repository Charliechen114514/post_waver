 ...
 * 知乎转换器选项
 */
export interface ZhihuTransformOptions {
  /** 文章ID */
  postId?: string
  /** 相关文章列表 */
...
/**
 * 将 Markdown 转换为知乎格式（Markdown）
 * 知乎原生支持 Markdown，所以主要是格式标准化和元数据提取
 */
export async function transformForZhihu(
  markdown: string,
  options?: ZhihuTransformOptions
): Promise<string> {
  let result = markdown
  ...
  if (options?.includeRelatedLinks && options.postId && options.relatedPosts) {
    ...
    const linksMarkdown = formatLinksAsMarkdown(links, 'zhihu')
    result = result + '\n\n---\n\n' + linksMarkdown
  }
